...
package com.cornucopia.tabcontainer;
...
import com.cornucopia.tabcontainer.animation.AnimationUtils;
import com.cornucopia.tabcontainer.internal.ThemeEnforcement;
import com.cornucopia.tabcontainer.resource.MaterialResources;
import com.cornucopia.tabcontainer.ripple.RippleUtils;
import com.cornucopia.tabcontainer.utils.ViewUtils;
...
public class TabLayout extends HorizontalScrollView {
  ...
  private  int tabIndicatorDiffOfLeft;
  private  int tabIndicatorDiffOfRight;
  ...
  public TabLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    tabIndicatorDiffOfLeft =
        a.getDimensionPixelSize(R.styleable.TabLayout_tabIndicatorDiffOfLeft, 0);
    tabIndicatorDiffOfRight =
        a.getDimensionPixelSize(R.styleable.TabLayout_tabIndicatorDiffOfRight, 0);
  ...
  class SlidingTabIndicator extends LinearLayout {
    ...
    private void calculateTabViewContentBounds(
      ...
      if (tabViewWidth != 0) {
        int tabViewContentOffset = (tabViewWidth - tabViewContentWidth) / 2;
        int selfTabViewContentWidth = tabViewContentWidth / 2;
        contentLeftBounds=contentLeftBounds+calculateTabIndicatorLeftDiff(-tabIndicatorDiffOfLeft,
            tabViewContentOffset,
            selfTabViewContentWidth);
        contentRightBounds=contentRightBounds+calculateTabIndicatorRightDiff(tabIndicatorDiffOfRight,
            tabViewContentOffset,selfTabViewContentWidth);
      }
      contentBounds.set(contentLeftBounds, 0, contentRightBounds, 0);
    }

    private int calculateTabIndicatorLeftDiff(int diff,
        int tabViewContentOffset, int selfTabViewContentWidth) {
      if (diff == 0) {
        return diff;
      } else if (diff > 0) {
        return diff > selfTabViewContentWidth ? selfTabViewContentWidth : diff;
      } else {
        return -diff > tabViewContentOffset ? -tabViewContentOffset : diff;
      }
    }

    private int calculateTabIndicatorRightDiff(int diff,
        int tabViewContentOffset, int selfTabViewContentWidth) {
      if (diff == 0) {
        return diff;
      } else if (diff < 0) {
  ...
  public int getTabIndicatorDiffOfLeft() {
    return tabIndicatorDiffOfLeft;
  }

  public void setTabIndicatorDiffOfLeft(int tabIndicatorDiffOfLeft) {
    this.tabIndicatorDiffOfLeft = tabIndicatorDiffOfLeft;
    applyModeAndGravity();
  }

  public int getTabIndicatorDiffOfRight() {
    return tabIndicatorDiffOfRight;
  }

  public void setTabIndicatorDiffOfRight(int tabIndicatorDiffOfRight) {
    this.tabIndicatorDiffOfRight = tabIndicatorDiffOfRight;
    applyModeAndGravity();
  }

  public void setTabTextSize(float tabTextSize){
     this.tabTextSize=ViewUtils.dpToPx(getContext(),(int)tabTextSize);
     applyModeAndGravity();
  }
